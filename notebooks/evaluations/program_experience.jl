...
Year: $(@bind program_year Select([2023, 2024, 2025]; default=2025))
"""
...
function sentiment_mat(df, sentiment_levels)
	stack(
		[
...
#grid(
  columns: (2fr, 1fr),
  [#text(17pt)[*Program Experience Survey\\ 2023 -- 2025*]],
  [#image("fig/logo.png")],
)
""";
...
df = @chain df_all begin
	...
	rename(s -> replace(s, r".*\[(.*)\].*" => s"\1"), _)
end

# ╔═╡ 0da528e1-22fe-4d6b-849c-df0f10e98dce
df_sentiment_student = @chain df begin 
	@select $(r"(The|this) student")
	# @select begin
...
df_sentiment_tutor = @chain df begin
	@select $(r"(The|this) tutor")
	# @select begin
...
let
	...
	save("src/fig/program.svg", fig, px_per_unit=3)
	fig
end

# ╔═╡ 50ef22c4-9319-4634-9e60-c034bde436c8
let
	open("src/program_experience.typ", "w") do report
		...
		write(report, "== Suggested improvements / Testimonial\n")
		response_text2(report, df, :"If you have any specific suggestions for improvement in the future or general testimonial, please enter it below.")
	end
	run(typst`compile src/program_experience.typ`)
end;
...
# ╔═╡ 3cc571c6-789e-45d7-898e-a15ff5d7d290
pos_mat = sentiment_mat(df_sentiment_student, sentiment_levels)'[:, end-1:end]

# ╔═╡ 0121c6a7-80b2-4778-82e6-072e0aec9ce3
# Percentage of respondents that agree Onaketa had
# a positive impact on their student
sum(pos_mat) / (sum(pos_mat[begin, :]) * size(pos_mat, 1))
...
const grade_map = let
	...
	for (i, halfgrade) in enumerate(string(g, '+') for g in 'A':'F')
		d[halfgrade] = i
	end
	d["N/A"] = 7
	d
...
# ╔═╡ 9945ebfc-8317-4cd6-b967-f04fd625d7cc
enumerate(string(g, '+') for g in 'A':'F') |> collect
...
let
	...
	save("src/fig/grade.svg", fig, px_per_unit=3)
	fig
end
...
response_text(df, :"If you have any specific suggestions for improvement in the future or general testimonial, please enter it below.")
...
manifest_format = "2.0"
project_hash = "51bc85b1ba6d8f1133b6a0ab48406a969129e79d"
